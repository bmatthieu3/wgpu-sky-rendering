...
where
    T: Float,
{
    fn new(min: Vector2<T>, max: Vector2<T>) -> Face<T> {
        Face { min, max }
    }

    fn split(self) -> [Face<T>; 4] {
    ...
    fn get_farthest_vertex(&self) -> (Vector2<T>, Direction) {
        let x_neg = self.min.x < T::zero();
        let y_neg = self.min.y < T::zero();
    ...
    fn get_nearest_vertex(&self) -> Vector2<T> {
        let x_neg = self.min.x < T::zero();
        let y_neg = self.min.y < T::zero();
    ...
    fn get_vertex(&self, d: Direction) -> Vector2<T> {
        match d {
    ...
    pub fn get_child(&self, d: Direction) -> Self {
        let center = (self.min + self.max) * T::from(0.5).unwrap();
        let (min, max) = match d {
            Direction::BottomLeft => {
                let min = self.min;
                let max = center;
